...
Details that make interfaces feel better. Great interfaces rarely come from a single thing. It's usually a collection of small details that compound into a great experience. Apply these principles when building or reviewing UI code.
...
| Animations | Interruptible transitions, scale on press, skipping animation on page load, motion restraint |
...
A standalone UI-polish review is finished when every confirmed finding is reported in the format in `review-output.md`, with verification and a verdict. Under `better-interface`, its format governs instead.
